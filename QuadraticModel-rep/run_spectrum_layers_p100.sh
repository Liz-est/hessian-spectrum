...
#
# 用法: bash run_spectrum_layers_p100.sh <STAMP> [CKPT]
...
for TAG in hessian_raw hessian_adam gn_raw gn_adam; do
  ...
  $PYTHON plot_full_spectrum.py --npz "$FULL_NPZ" \
      --outdir "$FIG" --name "full_${TAG}"
  RC[${TAG}_plot_full]=$?
  # 逐块：overlay + grid + summary
  $PYTHON plot_layers.py --npz "$BLK_NPZ" --outdir "$FIG"
  RC[${TAG}_plot_layers]=$?
...
echo ""
echo "======== 四曲线汇总图 ========"
$PYTHON plot_full_spectrum.py --npz ${OUT}/full_*.npz \
