...
# 用法: bash sco/run_spectrum_ddp_raw_completep.sh <pct>
set -e
REP_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$REP_ROOT"

PCT=$1
[ -z "$PCT" ] && { echo "用法: bash sco/run_spectrum_ddp_raw_completep.sh <pct>"; exit 1; }
...
if [ -n "$MASTER_ADDR" ] && [ -n "$RANK" ] && [ -n "$WORLD_SIZE" ]; then
    ...
    torchrun --standalone --nproc_per_node=8 -- src/spectrum/spectrum_ddp.py $COMMON
fi
