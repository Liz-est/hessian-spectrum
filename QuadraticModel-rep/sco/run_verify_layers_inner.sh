...
# 用法: bash sco/run_verify_layers_inner.sh
...
echo ""
echo "======== 1) src/analysis/verify_layers.py（数学对拍）========"
$PYTHON src/analysis/verify_layers.py; RC[1_verify]=$?
...
$TORCHRUN --standalone --nproc_per_node=8 -- src/spectrum/spectrum_layer_ddp.py \
    --ckpt "$CKPT" \
...
$TORCHRUN --standalone --nproc_per_node=8 -- src/spectrum/spectrum_ddp.py \
    --ckpt "$CKPT" \
...
$PYTHON scripts/plot_layers.py --npz test_outputs/smoke_blocks_p100.npz \
