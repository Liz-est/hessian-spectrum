...
# 故所有 GPU 相关验证走此脚本。依次：
#   1. src/optim/opt.py 自检（NS5/C₅ 一致性 + CompletePMuon）
#   2. config 切换
...
echo "======== 1. src/optim/opt.py 自检 ========"; date
$PY src/optim/opt.py
...
$TR --standalone --nproc_per_node=8 src/train/train.py smoke_muon
echo "======== 3b. smoke_adam 训练 ========"; date
$TR --standalone --nproc_per_node=8 src/train/train.py smoke_adam

echo "======== 4. spectrum_ddp muon ckpt (gn_muon) ========"; date
$TR --standalone --nproc_per_node=8 src/spectrum/spectrum_ddp.py \
