#!/bin/bash
# 提交 p10 谱计算（1 节点 × 8 卡，1M token + m=800）
...
# 检查是否已提交
if $SCO acp jobs list --workspace-name "$WS" 2>/dev/null | grep -q "spectrum-ddp-p10"; then
...
out=$($SCO acp jobs create \
  ...
  --command "bash $REP_ROOT/sco/run_spectrum_ddp_inner.sh p10 2>&1 | tee $REP_ROOT/test_outputs/spectrum_ddp_p10.log 2>&1" \
  2>&1)
