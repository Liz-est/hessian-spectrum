...
out=$($SCO acp jobs create \
  ...
  --job-name "$JOB" \
  --command "bash /data/250010020/hessian-spectrum/QuadraticModel-rep/run_spectrum_5m_inner.sh ${PCT} 2>&1 | tee /data/250010020/hessian-spectrum/QuadraticModel-rep/test_outputs/spectrum_hraw_5m_${PCT}.log" \
  2>&1)
