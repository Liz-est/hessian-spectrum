#!/usr/bin/env python3
"""
Submit one SCO job that runs the "simpliest" (embedding + lm_head only)
experiment on 8 GPUs: first DDP training, then the 8-GPU-sharded exact per-unit
Hessian analysis, all in a single job.

Usage (from the hessian-spectrum repo):
    python3 submit_sco_simpliest.py            # asks for confirmation
    python3 submit_sco_simpliest.py --yes      # submit without prompt
...
* Both scripts default to the "simpliest_sgd" preset (n_layer=0), so leaving
  EXP_ARGS empty runs the embed+head-only experiment.
* Phase 1 (train_simpliest_model.py) launches 8-GPU DDP via torchrun and writes one
  checkpoint per tag in the preset's ckpt_fracs under toy_models/runs/<run_name>/.
* Phase 2 (analyze_simpliest.py) launches 8-GPU torchrun; the (checkpoint x layer) work
...
TRAIN_LAUNCH = (
    ...
    f"--nproc_per_node={NPROC_PER_NODE} train_simpliest_model.py {TRAIN_ARGS}"
)
ANALYZE_LAUNCH = (
    f"{ENV_PYTHON} -u -m torch.distributed.run --standalone "
    f"--nproc_per_node={NPROC_PER_NODE} analyze_simpliest.py {ANALYZE_ARGS}"
)
