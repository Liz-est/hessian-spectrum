"""
Train the "simpliest" model (embedding + lm_head only, n_layer=0) on the
synthetic bigram data, log the loss curve, and checkpoint at the fractions of
training named in the experiment config's ckpt_fracs, so the Hessian analysis
(analyze_simpliest.py) can be run at each stage.

Defaults to the "simpliest_sgd" preset; all settings come from the config
package (config/schema.py + presets.py):

    python3 train_simpliest_model.py                      # simpliest_sgd preset
    python3 train_simpliest_model.py --train.max_iters=40 # quick smoke test
    python3 train_simpliest_model.py --optim.name=adamw --lr.learning_rate=3e-4
    torchrun --standalone --nproc_per_node=8 train_simpliest_model.py   # 8-GPU DDP
...
# ----------------------------------------------------------------------------
# config: default to the "simpliest_sgd" preset (embed + lm_head only, n_layer=0).
